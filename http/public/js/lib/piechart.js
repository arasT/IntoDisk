...
// Added to reinitialize chart value when user double click in the middle
function updateLabel() {
	var valueList = $(this).data('value');
	$(this).find('.flatmin-title').text(valueList[0]);
	$(this).find('.flatmin-percentage').text(valueList[1]);
	$(this).find('.flatmin-value').text(valueList[2]);
}

// Added to humanize size display convert size to TB, GB, MB, KB, B
...
(function ( $ ) {
		...
		/* Added:
			* Second parameter was added (f) to initialize default label
			* Third parameter was added (g) to get files extensions and color
		*/
    $.fn.piechart = function(e, f, g) {
			/*console.log(e);
			console.log(f);
			console.log(g);*/
    	chart = {};
    	...
    	var pieHole = $(['<div class="piehole" title="', f[0], ' ', f[1]+'"></div>'].join(""));			// Updated
		pieHole.on('mouseover', function(){$(this).attr('style','background:#303030; color:#FFF');})	// Added
				.on('mouseout', function(){$(this).attr('style','background:#FFF; color:#000');});				// Added
		pieHole.data('value', f);			// Added: To save default data from the second parameter
    	...
    	$.each(e,function(v,k) {
    		...
    		//var d = {color : get_random_color()};
				// Updated: use color from configuration files
				var fType = k[0];
				var d = { color : g[fType].color };
		...
		// Added to Initialize label in the center
		this.find('.piehole-text').attr('style','display:block');
		this.find('.flatmin-title').text(f[0]);
		this.find('.flatmin-percentage').text(f[1]);
		this.find('.flatmin-value').text(f[2]);
...
function paintChart(s,e) {
	//var sec = $('<div class="sector"></div>');	// Updated
	var sec = $(['<div class="sector" title="', s.nm, ': ', s.per.toFixed(2), '%"></div>'].join(""));
	sec.css(  offset(s.os) );
	e.append(sec);
	...
	if(s.per>50&&s.per<=100) {
		...
		secft.append(['<div class="sector-s" style="background-color:', s.color, '"></div>'].join(""));
		secft.css({"clip": ["rect(", 0, "px, ", h, "px, ", h, "px, ", (h/2), "px)"].join("")});
		sec.append(secft);
	}
}
...
function clickState() {
	...
	var val = readableSize(sector.data('val'));							// Added
	val = [val.size.toFixed(2), ' ', val.unit].join("");		// Added
	pieHole.find('.flatmin-value').text(val);								// Updated
	var sectorPercent = Math.round(sector.data('per')*100)/100;
	var sectorText = [sectorPercent, ' %'].join("");
	pieHole.find('.flatmin-percentage').text(sectorText); 	// Updated
...
function offset(o) {
	var s = {
		"transform": ["rotate(", o, "deg)"].join(""),
		"-ms-transform": ["rotate(", o, "deg)"].join(""),
		"-webkit-transform": ["rotate(", o, "deg)"].join("")
	};
	return s;
}
function percentToPie(s,h) {
	...
	o = {
		second: {
			"clip": ["rect(", cssopt.scliptop, "px, ", cssopt.sclipright, "px, ", cssopt.sclipbottom, "px, ", cssopt.sclipleft, "px)"].join(""),
			"transform": ["rotate(", cssopt.srotate, "deg)"].join(""),
			"-ms-transform": ["rotate(", cssopt.srotate, "deg)"].join(""),
			"-webkit-transform": ["rotate(", cssopt.srotate, "deg)"].join(""),
			background: s.color
		}, first: {
			"clip": ["rect(", cssopt.fcliptop, "px, ", cssopt.fclipright, "px, ", cssopt.fclipbottom, "px, ", cssopt.fclipleft, "px)"].join("")
		}
	};
	if(s.per>50&&s.per<=100) {
		o.first = {
			"clip": ["rect(", cssopt.fcliptop, "px, ", cssopt.fclipright, "px, ", cssopt.fclipbottom, "px, ", cssopt.fclipleft, "px)"].join(""),
			"transform": ["rotate(", cssopt.frotate, "deg)"].join(""),
			"-ms-transform": ["rotate(", cssopt.frotate, "deg)"].join(""),
			"-webkit-transform": ["rotate(", cssopt.frotate, "deg)"].join("")
		};
	}
	return o;
}
